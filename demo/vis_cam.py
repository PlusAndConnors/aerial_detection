...
from mmrotate.utils import (DetAblationLayer, DetBoxScoreTarget, DetCAMModel,
                         DetCAMVisualizer, FeatmapAM, reshape_transform)

try:
    from pytorch_grad_cam import AblationCAM, EigenCAM, GradCAM, GradCAMPlusPlus, XGradCAM, EigenGradCAM, LayerCAM
except ImportError:
...
def parse_args():
    ...
    parser.add_argument('--img_path', default='data/split_ms_thanos3/test', help='Image file')
    parser.add_argument('--config', default='', help='Config file')
    parser.add_argument('--checkpoint', default='work_dirs', help='Checkpoint file')
    parser.add_argument('--method', default='ablationcam', help='Type of method to use, supports 'f'{", ".join(list(METHOD_MAP.keys()))}.')
    parser.add_argument('--target-layers', default=['neck'], nargs='+', type=str,
        help='The target layers to get CAM, if not set, the tool will specify the neck')
    ...
    parser.add_argument('--max-reshape-shape', type=tuple, default=(20, 20), help='max reshape shapes. Its purpose is to save GPU memory. '
        'The activation map is scaled and then evaluated. If set to (-1, -1), it means no scaling.')
    parser.add_argument('--norm-in-bbox', action='store_true', help='No norm in bbox of cam image')
    ...
    parser.add_argument('--eigen-smooth', default=False, action='store_true',
        help='Reduce noise by taking the first principle componenet of '
        '``cam_weights*activations``')
    parser.add_argument('--out-dir', default='../cam_result', help='dir to output file')

    # Only used by AblationCAM
    parser.add_argument('--batch-size', type=int, default=1, help='batch of inference of AblationCAM')
    parser.add_argument('--ratio-channels-to-ablate', type=int,default=0.5, help='Making it much faster of AblationCAM. The parameter controls how many channels should be ablated')
    ...
    parser.add_argument('--cfg-options', nargs='+', action=DictAction, help='override some settings in the used config, the key-value pair '
        ...
        'It also allows nested list/tuple values, e.g. key="[(a,b),(c,d)]"  Note that the quotation marks are necessary and that no white space  is allowed.')
...
def main():
    args = parse_args()
    checkpoints = glob(args.checkpoint+'**/*.pth')
    images = glob(args.img_path+'/images/*')
