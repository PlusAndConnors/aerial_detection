...
class FocalModulation(nn.Module):
    ...
    def forward(self, x):
        ...
        ctx_global = self.act(ctx.mean(2, keepdim=True).mean(3, keepdim=True)) # 1,dim,256,256 -> 1, dim,1,1
        ctx_all = ctx_all + ctx_global * gates[:, self.focal_level:] # gate = 1,1,256,256
...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.proj(x)  # B C Wh Ww -- 1,96,256,256
...
class FocalNet(nn.Module):
    ...
    def __init__(self,
        ...
        for i_layer in range(self.num_layers):
            layer = BasicLayer(dim=int(embed_dim * 2 ** i_layer), depth=depths[i_layer], mlp_ratio=mlp_ratio,
                               drop=drop_rate, drop_path=dpr[sum(depths[:i_layer]):sum(depths[:i_layer + 1])],
                               norm_layer=norm_layer,
                               downsample=PatchEmbed if (i_layer < self.num_layers - 1) else None,
                               focal_window=focal_windows[i_layer], focal_level=focal_levels[i_layer],
                               use_conv_embed=use_conv_embed,
                               use_layerscale=use_layerscale, use_checkpoint=use_checkpoint)
    ...
    def forward(self, x):
        ...
        # if self.first:
        #     self.first = 0
        #     flops = FlopCountAnalysis(self.layers[0], x)
