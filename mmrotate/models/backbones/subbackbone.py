...
class LSKblock(nn.Module):
    def __init__(self, dim):
        ...
        self.f = nn.Linear(dim, 2, bias=True)

    def forward(self, x):
        attn1 = self.conv0(x)  # 2,32,256,256
        attn2 = self.conv_spatial(attn1)

        attn1 = self.conv1(attn1)  # 2,16,256,256
        attn2 = self.conv2(attn2)  # 2,16,256,256

        attn = torch.cat([attn1, attn2], dim=1)
        avg_attn = torch.mean(attn, dim=1, keepdim=True)
        max_attn, _ = torch.max(attn, dim=1, keepdim=True)
        agg = torch.cat([avg_attn, max_attn], dim=1)  # (batch,2,H,W)
        sig = self.conv_squeeze(agg).sigmoid()  # sig = (batch,2,H,W)
        attn = attn1 * sig[:, 0, :, :].unsqueeze(1) + attn2 * sig[:, 1, :, :].unsqueeze(1)
        attn = self.conv(attn)

        return x * attn  # 2,32,256,256 * 2,32,256,256
...
class MainSubblock(nn.Module):
    ...
    def forward(self, x, info=list()):
        if len(info)==0 or info[0].shape != x.shape:
            v = linear_p(x, self.v)
            info.append(v)
            for l in range(self.focal_level):
                v = self.focal_layers[l](v)
                info.append(v)
            info.append(self.act(v.mean(2, keepdim=True).mean(3, keepdim=True)))
        (_, C, _, _), ctx_all = x.shape, 0
        x1 = linear_p(x, self.f)
        q, gates = torch.split(x1, (C, self.focal_level + 1), 1)
        for l in range(self.focal_level):
            ctx_all = ctx_all + info[l] * gates[:, l:l + 1]
        out = q * self.h(ctx_all)
        return (out, info)

class Attention(nn.Module):
    def __init__(self, d_model, test_type=None, iters=0):
        super().__init__()
        self.test_type = test_type
        self.proj_1 = nn.Conv2d(d_model, d_model, 1)
        self.activation = nn.GELU()
        self.spatial_gating_unit = MainSubblock(d_model)
    ...
    def forward(self, x):
        ...
        x = self.proj_1(x)
        x = self.activation(x)
        x, self.gate_info = self.spatial_gating_unit(x, self.gate_info)
        x = self.proj_2(x)
        x = x + shorcut
        return x
...
class Block(nn.Module):
    def __init__(self, dim, mlp_ratio=4., drop=0., drop_path=0., act_layer=nn.GELU, norm_cfg=None,test_type=None, iters = None):
        super().__init__()
        if norm_cfg:
        ...
        self.attn = Attention(dim, test_type,iters)
    ...
    def __init__(self, img_size=224, in_chans=3, embed_dims=[64, 128, 256, 512],
        ...
        for i in range(num_stages):
            ...
            block = nn.ModuleList([Block(
                dim=embed_dims[i], mlp_ratio=mlp_ratios[i], drop=drop_rate, drop_path=dpr[cur + j], norm_cfg=norm_cfg, test_type=test_type, iters=j)
