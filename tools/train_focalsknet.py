...
def parse_args(): 
    parser = argparse.ArgumentParser(description='Train a detector')  # _deepblock
    parser.add_argument('--config', default='configs/lsknet/lsk-t-focalnet.py', help='train config file path')
    ...
    parser.add_argument(
        ...
        default='none',
        help='job launcher')
    parser.add_argument('--cuda_visible', type=int, default=0)
...
def main():
    ...
    if args.work_dir is not None:
        ...
        if args.my_test:
            ...
            cfg.work_dir = osp.join('./toy-project_dirs', name[-2]+str(datetime.datetime.now().day)+osp.splitext(name[-1])[0])
        else:
